// Inverted FFT spectral glitch waves //
...
volatile uint8_t glitch_rate = 0;
...
void calculate_next(int16_t* real_q15) {
    ...
    static uint16_t glitch_clock = 0;
    glitch_clock = glitch_clock + glitch_rate;

    uint16_t time_scramble = (glitch_clock >> BIN_GLITCH) & 1023;
    uint16_t glitch_offset = (time_scramble * 31) % (N / 2); 
    ...
    for (int bin = 1; bin < N / 2; bin++) {
        ...
        if (amp > 5) {
            ...
            uint16_t glitched_bin_idx = (bin + glitch_offset) % (N / 2);
            if (glitched_bin_idx == 0) glitched_bin_idx = 1;

            phase_accumulator[bin] += ((uint32_t)glitched_bin_idx + (glitch_clock & 15)) << LOG2_N;
            phase_accumulator[bin] &= 1023;
            
            uint16_t smeared_phase = (phase_accumulator[bin] ^ time_scramble) & 1023;
