// Inverse FFT spectral transmission engine //
...
void loop() { 
    ...
    static uint16_t dark_amp = 0;  
    static float chaos_phase = 0; 
    dark_counter++;

    if (dark_counter % 200 < 4) dark_amp = random(700, 1200);
    bin_amplitudes[random(1, 3)] = dark_amp; 

    if (dark_counter % 50 < 4) {
        chaos_phase += 0.005; 
        if (chaos_phase > 2 * M_PI) chaos_phase -= 2 * M_PI; 
     
        float chaos1 = sinf(chaos_phase); 
        float chaos2 = sinf(chaos_phase * 1.7 + 0.5); 
        float chaos3 = sinf(chaos_phase * 0.3 + 1.2); 
     
        int chaos_amp1 = (int)((chaos1 * 0.5 + 0.5) * 300); 
        int chaos_amp2 = (int)((chaos2 * 0.5 + 0.5) * 200); 
        int chaos_amp3 = (int)((chaos3 * 0.5 + 0.5) * 400); 
     
        bin_amplitudes[3] += random(10, chaos_amp1); 
        bin_amplitudes[6] += chaos_amp2; 
        bin_amplitudes[9] += chaos_amp3; 
        bin_amplitudes[12] += chaos_amp1 / 2; 
    }

    if (dark_counter % 1200 < 80) { 
        float pulse = sinf((dark_counter % 1200) / 1200.0f * 2 * M_PI); 
        int pulse_amp = (int)((pulse * 0.5 + 0.5) * 500); 
         
        bin_amplitudes[1] += pulse_amp; 
        bin_amplitudes[3] += pulse_amp / 2; 
        bin_amplitudes[5] += pulse_amp / 3; 
         
        if (pulse > 0.8) { 
            for (int i = 10; i < 20; i += 2) { 
                bin_amplitudes[i] += random(30, 120); 
            } 
        } 
    } 
    
    if (random(1000) < 6) { 
        int noise_type = random(0, 4); 
         
        switch (noise_type) { 
            case 0: 
                for (int i = 1; i < 8; i++) { 
                    bin_amplitudes[i] += random(20, 150); 
                } 
                break; 
                 
            case 1: 
                for (int i = 12; i < 25; i += 2) { 
                    bin_amplitudes[i] += random(30, 100); 
                } 
                break; 
                 
            case 2: 
                for (int i = 25; i < 35; i += 3) { 
                    bin_amplitudes[i] += random(10, 70); 
                } 
                break; 
                 
            case 3: 
                for (int i = 1; i < 50; i += 4) { 
                    bin_amplitudes[i] += random(10, 75); 
                } 
                break; 
        } 
    } 
    ...
    if (random(500) < 2) { 
        int voice_base = 6 + random(0, 8); 
        int voice_amp = 200 + random(0, 300); 
         
        for (int i = 0; i < 5; i++) { 
            int bin = voice_base + i * 2; 
            if (bin < N / 2) { 
                bin_amplitudes[bin] += voice_amp / (i + 1); 
                if (i % 2 == 0) { 
                    bin_amplitudes[bin + 1] += voice_amp / (i + 2); 
                } 
            } 
        } 
    } 
    ...
    if (random(100) < 2) { 
        int quake_amp = 350 + random(0, 650); 
         
        for (int i = 1; i < 5; i++) { 
            bin_amplitudes[i] += quake_amp / i; 
            if (i * 2 < N / 2) { 
                bin_amplitudes[i * 2] += quake_amp / (i * 2); 
            } 
        } 
    } 
