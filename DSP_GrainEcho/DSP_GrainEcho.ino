// Granular ambient echo drone //

#define SAMPLE_RATE   44100
#define BUFFER_SIZE   256 
#define DELAY_BUFFER  4096
...
void setup() {
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    ...
    float modFreq = 2.71f;
    float modIndex = 3.5f;
    float fmWave = sinf(t * carFreq + sinf(t * modFreq) * modIndex);
    float noise = ((random(0, 1000) / 500.0f) - 1.0f) * 0.2f;
    audioBuffer[i] = (uint16_t)(512.0f + 511.0f * tanhf(fmWave * (1.0f + noise)));
  }
...
void loop() {
  ...
  uint16_t currentLength = random(BUFFER_SIZE / 4, BUFFER_SIZE);
  grainLength = currentLength << 8;
  nextGrainResetPosition = random(0, 16) << 8;

  wowPhase += 0.01f;
  if (wowPhase > 2 * PI) wowPhase -= 2 * PI;
  float wowMod = sinf(wowPhase) * 45.0f;

  flutterPhase += 0.08f;
  if (flutterPhase > 2 * PI) flutterPhase -= 2 * PI;
  float flutterMod = cosf(flutterPhase) * 4.0f; 

  uint16_t baseSpeed = random(24, 128);
  grainSpeed = (uint16_t)((float)baseSpeed + wowMod) << 5;
  currentDelayMax = (DELAY_BUFFER - 20) + (int16_t)flutterMod;
  feedbackGain = 90 + random(0, 8);

  delay(random(800, 3500));
