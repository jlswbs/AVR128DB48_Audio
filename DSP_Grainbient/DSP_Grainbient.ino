...
void setup() {
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    ...
    audioBuffer[i] = (uint16_t)(512.0f + 511.0f * tanhf(8.0f * (sinf(t) * (1.0f + noise))));
  }
...
void loop() {
  ...
  uint16_t currentLength = random(128, BUFFER_SIZE);
  grainLength = currentLength << 8;

  uint16_t baseSpeed = (random(0, 10) > 7) ? 256 : 512;

  wowPhase += 0.005f;
  if (wowPhase > 2 * PI) wowPhase -= 2 * PI;
  float wowMod = sinf(wowPhase) * 25.0f;
  ...
  currentDelayMax = (DELAY_BUFFER - 4) + (int16_t)flutterMod;
  feedbackGain = random(96, 99); 

  delay(random(400, 1600));
