// Inverted FFT spectral shift chimes //
...
void calculate_next(int16_t* real_q15) {
    ...
    for (int bin = 1; bin < N / 2; bin++) {
        ...
        if (amp > 5) {
            ...
            int16_t neighbor_amp = (bin > 1) ? bin_amplitudes[bin - 1] : bin_amplitudes[1];
            uint16_t phase_deviation = (uint16_t)(neighbor_amp >> BIN_SHIFT);
    ...
    if (random(100) < 10) {
        ...
        for (int i = 0; i < N / 2; i++) {
            ...
            if (bin_amplitudes[i] > 0) {
                bin_amplitudes[i] = (bin_amplitudes[i] * 95) / 100;
