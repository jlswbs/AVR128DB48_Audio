// Chip glitch sound engine with reverse delay //
...
uint8_t glitchram[SONGDATA_SIZE];

#define DELAY_SIZE 2048
int16_t delay_buffer[DELAY_SIZE];
uint16_t delay_pos = 0;
uint16_t delay_len = 2000;
...
int16_t chiptune_callback() {
    ...
    int16_t read_pos = (DELAY_SIZE - 1 - delay_pos) % DELAY_SIZE;
    int16_t delayed = delay_buffer[read_pos];

    delay_buffer[delay_pos] = acc >> 1;

    delay_pos++;
    if (delay_pos >= DELAY_SIZE) delay_pos = 0;

    int32_t output = acc + (delayed >> 3);
...
ISR(TCB0_INT_vect) {
    ...
    uint16_t val = 512 + (chiptune_callback() >> 6);
