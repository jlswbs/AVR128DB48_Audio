// Inverted FFT spectral comb metal //
...
static uint32_t rand_state = 0xACE1; 
...
void calculate_next(int16_t* real_q15) {
    ...
    comb_phase_offset = (comb_phase_offset + 1) & 1023; 
...
void loop() {
    ...
    if (random(1000) < 5) { 
        int strike_type = random(3);

        if (strike_type == 0) {
            for (int i = 1; i < N / 2; i++) {
                bin_amplitudes[i] = random(150, 1250); 
            }
        } 
        else if (strike_type == 1) {
            for (int i = 1; i < N / 4; i++) {
               bin_amplitudes[i] = random(350, 1750);
            }
        }
        else {
            int center_bin = random(5, N / 2 - 10);
            for (int i = center_bin - 4; i <= center_bin + 4; i++) {
                bin_amplitudes[i] = random(250, 1450);
            }
        }
    }
    ...
    for (int i = 0; i < N / 2; i++) {
        if (bin_amplitudes[i] > 0) {
            bin_amplitudes[i] = (bin_amplitudes[i] * 96) / 100;
        }
    }
