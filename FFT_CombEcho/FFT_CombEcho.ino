// Inverted FFT spectral comb echo //
...
void calculate_next(int16_t* real_q15) {
    ...
    static uint16_t comb_phase_offset = 0;
    comb_phase_offset = (comb_phase_offset + 3) & 1023; 

    const uint8_t comb_density = BIN_COMB;
    const uint16_t delay_time = 16; 
    const int16_t echo_feedback = 18000; 
    ...
    for (int bin = 1; bin < N / 2; bin++) {
        ...
        if (amp > 5) {
            ...
            uint16_t comb_idx = ((bin * comb_density * 1024) >> LOG2_N) + comb_phase_offset;
            comb_idx &= 1023;

            int16_t comb_cos, comb_sin;
            get_twiddle(comb_idx, &comb_cos, &comb_sin);

            int16_t comb_modifier = (comb_sin < 0) ? -comb_sin : comb_sin; 
            amp = ((int32_t)amp * comb_modifier) >> 15;
            ...
            uint16_t echo_phase = (phase_accumulator[bin] + (bin * delay_time)) & 1023;
            
            int16_t cos_e, sin_e;
            get_twiddle(echo_phase, &cos_e, &sin_e);

            int16_t echo_amp = ((int32_t)amp * echo_feedback) >> 15;
            int32_t mixed_re = ((int32_t)amp * cos_p + (int32_t)echo_amp * cos_e) >> 15;
            int32_t mixed_im = ((int32_t)amp * sin_p + (int32_t)echo_amp * sin_e) >> 15;

            real_q15[bin] = mixed_re;
            imag_q15[bin] = mixed_im;
    ...
    if (random(100) < 9) {
        ...
        switch (bin % 6) {
            case 0:
                ...
                break;
            case 1:
                bin_amplitudes[bin] >>= 2;
                break;
            case 2:
                bin_amplitudes[bin / 2] = random(0, 2800);
                break;
            case 3:
                if (bin < N / 2 - 1)
        ...
        for (int i = 0; i < N / 2; i++) {
            ...
            if (bin_amplitudes[i] > 0) {
                bin_amplitudes[i] = (bin_amplitudes[i] * 98) / 100;
