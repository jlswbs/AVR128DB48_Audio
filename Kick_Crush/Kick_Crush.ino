// Karplus-Strong kick crush drum //
...
volatile uint8_t kickCrushAmt = 0;
int16_t burstFilterLast = 0;
...
void triggerKick(float frequency) {
  ...
  int16_t halfLength = delayLength >> 1;
  
  for (int i = 0; i < delayLength; i++) {
    if (i < halfLength) {
      delayBuffer[i] = (i < (halfLength >> 1)) ? (i * (1024 / halfLength)) : (511 - (i * (1024 / halfLength)));
    } else {
      int16_t i2 = i - halfLength;
      delayBuffer[i] = (i2 < (halfLength >> 1)) ? (-i2 * (1024 / halfLength)) : (-512 + (i2 * (1024 / halfLength)));
    }
  }
...
void loop() {
  ...
  feedbackGain = 20000 + random(0, 7000);
  kickCrushAmt = random(0, 128); 
...
ISR(TCB0_INT_vect) {
  ...
  int16_t finalSample = filteredSample;
  uint16_t mask = 0xFFFF ^ kickCrushAmt;
  finalSample = finalSample & mask;

  uint16_t dacValue = 512 + finalSample;
